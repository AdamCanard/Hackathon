...
searchSubmit.addEventListener("click", fetchCourseJson);
searchBar.addEventListener("input", search);

async function fetchCourseJson(input) {
  console.log(input);
  let temp = await fetch("http://server.jgaribsin.com:3000/courses/findAll", {
    headers: {},
    method: "GET",
  });
  return temp.json();
...
async function search() {
  ...
  data = await fetchCourseJson();
  let courses = data.data.courses;
  console.log(courses);

  for (let i = 0; i < courses.length; i++) {
    if (courses[i].toLowerCase().includes(searchBar.value) && searchBar.value) {
      newElem = document.createElement("a");
      newElem.href =
        "/pages/course.html?code=" + encodeURIComponent(courses[i]);
      newElem.classList.add("course-anchor");
      newElem.onclick = loadCoursePage;
      newElem.textContent = courses[i];
